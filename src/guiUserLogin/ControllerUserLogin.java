...
public class ControllerUserLogin {
	...
	*/
	...
	protected static void doLogin(Stage ts) {
    	...
    	// If the user has Admin privileges, go straight to Admin Home.
    	// This ignores if they have multiple roles and forces the Admin View.
    	if (user.getAdminRole()) {
			loginResult = theDatabase.loginAdmin(user);
			if (loginResult) {
				guiAdminHome.ViewAdminHome.displayAdminHome(theStage, user);
				return; // Stop execution here so we don't trigger the logic below
			}
    	}
		...
		if (numberOfRoles == 1) {
			...
			if (user.getNewRole1()) {
				...
				// Fallback if role count is 1 but no specific role flags matched
			...
			// This catches users who have 0 roles (like the one you just created)
